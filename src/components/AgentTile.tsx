...
// The Team Lead's tile (manager board only) carries the role and no break budget.
...
export function AgentTile({ tile, nowMs, multiplier, href }: { tile: TileVM; nowMs: number; multiplier: number; href?: string }) {
  ...
  const isLead = tile.kind === "team_lead";
  const note =
    tile.note?.kind === "away"
  ...
  const body = (
    <>
      ...
      <div className="flex items-center justify-between gap-2">
        <span className="min-w-0 flex flex-col leading-[1.3]">
          <span className="text-lg font-semibold whitespace-nowrap overflow-hidden text-ellipsis">{tile.name}</span>
          {isLead && <span className="text-[13px] text-muted">{ar.roles.team_lead}</span>}
        </span>
  ...
  const data = { "data-agent": tile.userId, "data-agent-name": tile.name, "data-status": status, "data-budget": tile.budgetUsed, "data-kind": tile.kind };
  if (href) {
    return (
